...
create table AE_spread as (
	select user_id, AE_numerator::real/posts_no_AE_denominator as engagement, distinct_actors_no_spread  as spread, posts_no_AE_denominator as posts_num from summary_post_counts_per_user order by posts_no_AE_denominator,distinct_actors_no_spread desc
);

COPY (select * from AE_spread) TO '/Users/admin/Desktop/data/opendata_klout/data/pinterest/user_id_engagement_spread_weights1.csv' DELIMITER ',' CSV HEADER;
...
create table NONLINEAR_weight2 as(
select user_id, post_id, actor_id, 2*repins_no+1*likes_no+1*repins_no*likes_no as nonlinear_val from user_actor_likes_repins
);


create table PRI_weight2 as(
select user_id, post_id, sum(nonlinear_val) as pri_numerator, count(distinct actor_id) as pri_denominator from NONLINEAR_weight2 group by user_id, post_id
);

alter table PRI_weight2 add column PRI_value real default 0;

update PRI_weight2 set PRI_value=i.prival from (select user_id, post_id, pri_numerator::real/pri_denominator as prival from pri_weight2 ) i where i.user_id = pri_weight2.user_id and i.post_id=pri_weight2.post_id;


create table summary_post_counts_per_user_weight2 as (
select 	user_id, count(distinct post_id) as posts_no_AE_denominator, count(distinct actor_id) as distinct_actors_no_spread from joined_table group by user_id order by count(post_id)
);
alter table summary_post_counts_per_user_weight2 add column AE_numerator int default 0;

update summary_post_counts_per_user_weight2 set AE_numerator=i.AE_numerator from (select user_id, sum(PRI_value) as AE_numerator from PRI_weight2 group by user_id) i where i.user_id = summary_post_counts_per_user_weight2.user_id;


create table AE_spread_weight2 as (
	select user_id, AE_numerator::real/posts_no_AE_denominator as engagement, distinct_actors_no_spread  as spread, posts_no_AE_denominator as posts_num from summary_post_counts_per_user_weight2 order by posts_no_AE_denominator,distinct_actors_no_spread desc
);
